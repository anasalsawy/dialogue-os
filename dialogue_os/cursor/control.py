"""Cursor control-plane: every Telegram ingress passes through Cursor first."""
...
log = get_logger("cursor.control")

CONTROL_PROMPT_PREAMBLE = """You are the Dialogue-OS / YTA Cursor control plane (Chief).

Architecture rules (binding):
- You (Cursor CLI) are Chief and the universal Telegram control plane.
...
- Microsoft Agent Framework may help with handoffs; it must not replace Hermes/Cursor identity.
...
class ControlPlane:
    def __init__(self, sessions: CursorSessionManager, store: Store):
    ...
    async def handle_event(self, event: dict[str, Any], on_partial=None) -> ControlDecision:
        ...
        if not result.ok:
            return ControlDecision(
                action="respond",
                response_bot=event.get("destination_agent") or "chief",
                text=f"Cursor control plane error: {result.error or 'unknown failure'}",
                ok=False,
                error=result.error,
    ...
    async def chief_direct(
        ...
        """Chief private chat: Cursor answers as itself (still control session)."""
        prompt = (
        ...
        if not result.ok:
            return ControlDecision(
                action="respond",
                response_bot="chief",
                text=f"Cursor error: {result.error or 'failed'}",
                ok=False,
                error=result.error,
                cursor_session_id=result.session_id,
                duration_seconds=result.duration_seconds,
...
def parse_control_decision(text: str, default_bot: str = "chief") -> ControlDecision:
    ...
    if not data:
        # Treat free-form Cursor output as a direct Chief/agent response
        return ControlDecision(
            action="respond",
