"""Persistent Cursor control-plane session management."""

from __future__ import annotations

from pathlib import Path

from dialogue_os.cursor.client import CursorClient, CursorResult
from dialogue_os.db.store import Store
from dialogue_os.util.logging import get_logger

log = get_logger("cursor.sessions")


class CursorSessionManager:
    def __init__(self, store: Store, client: CursorClient, primary_key: str = "primary"):
        self.store = store
        self.client = client
    ...
    async def ensure_primary(self) -> str:
        ...
        log.info("cursor_session_created", key=self.primary_key, session_id=session_id)
        return session_id
    ...
    async def set_active(self, key: str) -> str:
        ...
        if not sid:
            raise KeyError(f"No Cursor session for key={key}")
